...
Theorem app_assoc : forall A (l m n:list A),
  l ++ m ++ n = (l ++ m) ++ n.
Proof. intros X l m n. induction l as [|h t IH].
       - simpl. reflexivity.
...
Proof. intros X l1 l2. induction l1 as [|h t IH].
       - simpl. reflexivity.
       - simpl. rewrite -> IH. reflexivity.
Qed.
...
Theorem rev_involutive : forall X : Type, forall l : list X,
  rev (rev l) = l.
Proof. intros X l. induction l as [|h t IH].
       - simpl. reflexivity.
...
Proof. intros X Y f l1 l2. induction l1 as [|h t IH].
       - simpl. reflexivity.
       - simpl. rewrite -> IH. reflexivity.
Qed.  
...
Proof. intros X Y f l. induction l as [|h t IH].
       - simpl. reflexivity.
...
Definition fold_map {X Y:Type} (f : X -> Y) (l : list X) : list Y :=
  (* fold (fun x l' => app l' [f x]) l []. *)
  (* rev (fold (fun x l' => f x :: l' ) l []). *)
...
Theorem fold_map_nil: forall X Y (f: X-> Y),
    fold_map f [] = [].
Proof. reflexivity. Qed.

(* Theorem fold_map_assoc : forall X Y (f:X->Y) (l1 : list X) (l2 : list X), *)
    (* fold_map f (l1 ++ l2) = fold_map f l1 ++ fold_map f l2. *)
(* Proof. intros X Y f l1 l2. induction l2 as [|h t IH]. *)
       (* - simpl. rewrite -> app_nil_r. simpl. rewrite -> fold_map_nil. rewrite->app_nil_r. reflexivity. *)
         (* - simpl. *)
...
Proof. intros X Y Z f p. destruct p as [x y].
       reflexivity.
Qed.
(** [] *)
